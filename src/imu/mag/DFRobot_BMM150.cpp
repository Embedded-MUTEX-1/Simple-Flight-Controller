...
int DFRobot_BMM150::getOperationMode(void)
{
  uint8_t regData = 0;
  uint8_t powerData = 0;
  uint8_t number = 0;
...
int DFRobot_BMM150::getMeasurementStateXYZ(void)
{
  uint8_t regData = 0;
  getReg(BMM150_REG_AXES_ENABLE, &regData, 1);
  regData = (regData&0x38)>>3;
...
int DFRobot_BMM150::selfTest(uint8_t testMode)
{
  int8_t rslt;
  int result;
  switch (testMode)
  ...
  if(rslt == SELF_TEST_XYZ_SUCCESS){
    ...
    if((rslt>>0)&0x01) result += 1;
    if((rslt>>1)&0x01) result += 1;
    if((rslt>>2)&0x01) result += 1;
  }
  return result;
}
...
DFRobot_BMM150_I2C::DFRobot_BMM150_I2C(I2C *pWire, uint8_t addr)
{
  _pWire = pWire;
  this->_I2C_addr = addr;
}
...
{
  if(_pWire->write(this->_I2C_addr << 1, nullptr, 0) == 0){
    if(sensorInit()){
      return 0;
    }else{
...
void DFRobot_BMM150_I2C::writeData(uint8_t Reg, uint8_t *Data, uint8_t len)
{
    ...
    if(_pWire->write(this->_I2C_addr << 1, buf, len + 1, false) != 0) {
        return;
    }
}
...
{
  _pWire->write(this->_I2C_addr << 1, (const char *)&Reg, 1, false);
  _pWire->read(this->_I2C_addr << 1, (char *)Data, len);
  return 0;
}