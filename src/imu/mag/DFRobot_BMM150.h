...
public:
   ...
   * @return result Return sensor operation mode as a character s
   */
   ...
   * @return result Return enabling status as a character s
   */
  int getMeasurementStateXYZ(void);
   ...
   * @n s state The storage state is binary data s
   ...
   * @return result The returned character s is the self test result
   */
  int selfTest(uint8_t testMode);
...
protected:
  ...
  /**
   * @fn binChanges
   * @brief Convert binary value to character s
   * @param value 
   * @return The character s with the same expression as the binary
   */
  int binChanges(uint8_t value);
...
public:
  DFRobot_BMM150_I2C(I2C *pWire, uint8_t addr = 0x75);
  uint8_t begin(void);
protected:
