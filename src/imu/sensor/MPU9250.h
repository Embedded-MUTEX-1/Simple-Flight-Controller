...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), I2C* w = nullptr) {
...
private:
    void initMPU9250() {
        ...
        ThisThread::sleep_for(100ms);                                  // Wait for all registers to reset
...
private:
    ...
    void set_acc_gyro_to_calibration() {
        ...
        ThisThread::sleep_for(40ms);                                  // accumulate 40 samples in 40 milliseconds = 480 bytes
    }
    ...
    void collect_mag_data_to(float* m_bias, float* m_scale) {
        ...
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            ...
            if (MAG_MODE == 0x02) ThisThread::sleep_for(135ms);  // at 8 Hz ODR, new mag data is available every 125 ms
            if (MAG_MODE == 0x06) ThisThread::sleep_for(12ms);   // at 100 Hz ODR, new mag data is available every 10 ms
        }
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress) {
        ...
        wire->write(address << 1, (const char *)&subAddress, 1, false);
    ...
    void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t* dest) {
        wire->write(address << 1, (const char *)&subAddress, 1, false);
        wire->read(address << 1, (char *)dest, count);
    }
